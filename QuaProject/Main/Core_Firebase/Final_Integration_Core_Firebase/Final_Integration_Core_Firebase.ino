...
void setup() {
  ...
  Serial.println("Firebase integration - data logging every 30 seconds");
...
void sendDataToFirebase() {
  ...
  if (Firebase.RTDB.setString(&fbdo, basePath + "/rainfall/intensity_status", rainIntensityStr)) {
    Serial.println("✓ Rain intensity sent successfully");
  } else {
  ...
  if (Firebase.RTDB.setFloat(&fbdo, basePath + "/rainfall/total_mm", totalRainfall)) {
    Serial.println("✓ Total rainfall sent successfully");
  }
  
  // Send Float Sensor Data - FIXED LOGIC
  if (Firebase.RTDB.setString(&fbdo, basePath + "/float/water_level", floatStatusStr)) {
    Serial.println("✓ Float status sent successfully: " + floatStatusStr);
  } else {
    Serial.println("✗ Failed to send float status: " + fbdo.errorReason());
  }
  
  // Send Float Alarm Status - FIXED LOGIC
  if (Firebase.RTDB.setBool(&fbdo, basePath + "/float/alarm_active", floatAlarm)) {
    Serial.println("✓ Float alarm status sent successfully: " + String(floatAlarm ? "true" : "false"));
  } else {
    Serial.println("✗ Failed to send float alarm: " + fbdo.errorReason());
  }
  
  // Send Float Sensor Raw Data for debugging
  if (Firebase.RTDB.setInt(&fbdo, basePath + "/float/raw_gpio_reading", confirmedState)) {
    Serial.println("✓ Float raw GPIO sent successfully: " + String(confirmedState));
  }
  
  // Send Float Confirmation Status
  if (Firebase.RTDB.setBool(&fbdo, basePath + "/float/state_confirmed", stateConfirmed)) {
    Serial.println("✓ Float confirmation status sent successfully: " + String(stateConfirmed ? "true" : "false"));
  }
  ...
  if (Firebase.RTDB.setString(&fbdo, basePath + "/system/device", "BI-HYDRO-TECH")) {
    Serial.println("✓ Device info sent successfully");
  }
  
  // Send Firebase Upload Timestamp
  if (Firebase.RTDB.setInt(&fbdo, basePath + "/system/upload_time", timestamp)) {
    Serial.println("✓ Upload timestamp sent successfully");
  }
  
  Serial.println("Firebase data upload completed!");
}
...
void sendFloatStatusToFirebase() {
  ...
  Serial.println("=== IMMEDIATE FLOAT UPDATE ===");
...
void updateFloatSensorNonBlocking(unsigned long currentTime) {
  ...
  // Debug: Print raw reading every 2 seconds
  static unsigned long lastDebugPrint = 0;
  if (currentTime - lastDebugPrint > 2000) {
    Serial.print("FLOAT DEBUG - Raw GPIO reading: ");
    Serial.print(newReading);
    Serial.print(" (");
    Serial.print(newReading == HIGH ? "HIGH" : "LOW");
    Serial.println(")");
    Serial.print("Float interpretation: ");
  ...
  if (!stateConfirmed && (currentTime - stateStartTime) >= MIN_STATE_TIME) {
    ...
    if (confirmedState != lastConfirmedState) {
      ...
      // IMMEDIATE Firebase update for float changes
      if (wifiConnected && Firebase.ready() && signupOK) {
        Serial.println("Sending immediate float status update to Firebase...");
